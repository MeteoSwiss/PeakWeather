...
class PeakWeatherDataset:
    ...
    def __init__(
        ...
        if parameters is not None:
            ...
            if "wind_u" in parameters or "wind_v" in parameters:
                assert compute_uv, (
                    "Parameters `wind_u` and `wind_v` require setting "
                    "`compute_uv=True`."
                )
    ...
    def required_file_names(self) -> Mapping[str, str]:
        ...
        out.update({str(year): f"observations/{year}.parquet" for year in self.years})
        out.update(
            {
    ...
    def _maybe_unzip(self, filepath) -> None:
        if os.path.exists(filepath) and filepath.endswith(".zip"):
            extract_zip(path=filepath, folder=filepath.split(".zip")[0])
            os.remove(filepath)
    ...
    def load_topography(self) -> dict:
        ...
        topography = {
            k: xr.open_zarr(v)
            for k, v in self.required_files_paths.items()
    ...
    def _recompute_dir_from_uv(
        ...
        new_wind_dir = PeakWeatherDataset.get_wind_direction(u=u.values, v=v.values)
    ...
    def resample(
        ...
        if "wind_direction" in self.parameter_map and not self.compute_uv:
            ...
            wind_aggr = df_parameters.loc["wind_speed", "aggregation"]
    ...
    def load_raw(self, aggregation_methods: Optional[dict[str, str]] = None):
        ...
        if self.station_type is not None:
            ...
            df_observations = df_observations.loc[:, (common_stations, slice(None))]
    ...
    def interpolate_topography(
        ...
        for topo_name, topo_ds in topographic_params.items():
            if self.interpolation_method == "nearest":
                ...
                topo_interp = topo_ds.interp(
                    x=x_coords, y=y_coords, method=self.interpolation_method
                )
            topography_dict.update(
                {var: topo_interp[var].values for var in topo_interp.data_vars}
            )
    ...
    def show_parameters_description(self):
        ...
        pretty_df["Description"] = (
            pretty_df["description_E"] + " (" + pretty_df["unit_name_E"] + ")"
        )
    ...
    def get_observation_windows(
        ...
        windows = Windows(
            x=x, mask_x=mask_x, y=y, mask_y=mask_y, index_x=index_x, index_y=index_y
        )
    ...
    def np_windows_as_xr(
        ...
        windows_xr = self._pack_windows_as_xarray(
            windows,
    ...
    def align_windows(
        ...
        if not (set(nwp_pars) <= set(y_pars_init)):
            raise ValueError(
                "Aligning NWP forecasts with observations is possible only when the "
                "NWP parameters are also loaded as observations. "
                f"The requested NWP parameters are {nwp_pars}, while the observation "
                "parameters are {y_pars_init}."
            )
    ...
    def get_windows(
        ...
        Args:
            ...
            last_date (str or pd.Timestamp, optional): End date for filtering.
                If :obj:`None`, no temporal filtering is applied.
                (default: :obj:`None`)
            split (Literal['train', 'test', 'nwp_test'], optional): Predefined data
                split to load. If given, `first_date` and `last_date` must be None.
                (default: :obj:`None`)
            nwp_parameters (str or list, optional): Defines which NWP parameters to
                return. If no parameter is specified, none will be returned. If split
                is 'nwp_test', an NWP parameter must be specified. The windows will then
                then be aligned on the reference foreasting time available from
                the NWP model ICON-CH1-EPS, i.e., every 3 hours starting at midnight
                UTC. If a list is given, all those parameters are loaded. The nwp data
                is then available in Windows.nwp. If no split is specified but
                nwp_parameter is not None, the windows will nonetheless be aligned with
                the first available timestamp of the nwp data.
                (default: :obj:`None`)
            drop_extra_y_pars (bool, optional): If :obj:`False`, the returned parameters
                in `y` are selected according to `parameters`, as per `x`.
                If :obj:`True`, only the features corresponding to `nwp_parameter` are
                returned.
                (default: :obj:`True`)
        ...
        if split is not None and split not in ["train", "test", "nwp_test"]:
            raise ValueError(
                f"Invalid split value: {split}. Expected 'train', 'test' or 'nwp_test'."
            )

        need_nwp_data = split == "nwp_test" or nwp_parameters is not None
        if need_nwp_data:
            assert nwp_parameters is not None, (
                "For split=nwp_test you must specify an ICON parameter to load."
            )

            freq = pd.infer_freq(self.observations.index)
            assert pd.tseries.frequencies.to_offset(freq) == pd.Timedelta("1h"), (
                "The loaded observations have a frequency of "
                f"{self.observations.index.freqstr.lower()}, while "
                "the ICON forecasts are only available at 1h frequency. "
                'Please reload the dataset with the flag freq="h"'
            )

            assert horizon_size <= 33, (
                "ICON-CH1-EPS data only forecasts up to 33h ahead. "
                "Use a smaller horizon size."
            )
        ...
        if need_nwp_data:
            ...
            windows, nwp_aligned = self.align_windows(
                windows, nwp_data, drop_extra_y_pars, as_xarray
            )
            windows.nwp = nwp_aligned

        return windows

    def _get_windows_coordinates(
        self, windows: Windows, stations=None, parameters=None
    ):
    ...
    def _pack_windows_as_xarray(
        ...
        def get_data_vars(data, feature, x_or_y: str):
            ...
            return {
                f: (["reftime", second_dim, "nat_abbr"], data[..., i])
        ...
        x = xr.Dataset(
            ...
            coords={"reftime": reftime, "lag": lagtime, "nat_abbr": stations},
            attrs={
                "parameter": (
                    ", ".join(feature) if not isinstance(feature, str) else feature
                ),
                "description": (
                    "Represents past measurements with respect to each reftime"
                ),
                **common_attrs,
            },
        )
        ...
        mask_x = xr.Dataset(
            ...
            attrs={
                "parameter": (
                    ", ".join(f"(mask){v}" for v in feature)
                    if not isinstance(feature, str)
                    else feature
                ),
                "description": (
                    "Represents the availability mask of past measurements with "
                    "respect to each reftime"
                ),
                **common_attrs,
            },
        )
        ...
        y = xr.Dataset(
            ...
            coords={"reftime": reftime, "lead": leadtime, "nat_abbr": stations},
            attrs={
                "parameter": (
                    ", ".join(feature) if not isinstance(feature, str) else feature
                ),
                "description": (
                    "Represents future measurements with respect to each reftime"
                ),
                **common_attrs,
            },
        )
        ...
        mask_y = xr.Dataset(
            ...
            attrs={
                ...
                "description": (
                    "Represents the availability mask of future measurements with "
                    "respect to each reftime"
                ),
                **common_attrs,
            },
    ...
    def load_icon_data(self, icon_parameter: Union[str, Sequence[str]]) -> "xr.Dataset":
        ...
        if icon_parameter in self.available_icon:
            xr = import_xarray()
            return xr.open_zarr(
                self.required_files_paths[f"icon_{icon_parameter}"],
                decode_timedelta=True,
            )
        else:
            raise ValueError(
                f"Can not load ICON parameter {icon_parameter}. "
