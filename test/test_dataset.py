...
def test_dataset_shapes():
    ...
    obs = ds.get_observations(
        stations=["ABO", "KLO", "GRO"], parameters="wind_speed", as_numpy=True
    )
...
def test_temporal_subset():
    ...
    ds_meteo = PeakWeatherDataset(
        root=TEST_DATA_ROOT, freq="h", extended_nwp_pars="all"
    )
...
def test_station_type():
    ...
    assert not set(ds_station.stations).intersection(set(ds_gauge.stations)), (
        "Intersection between gauges and stations should be empty."
    )


def test_dataset_computes_uv():
    ds = PeakWeatherDataset(root=TEST_DATA_ROOT, freq="h", years=2017, compute_uv=True)
...
def test_splits():
    ds = PeakWeatherDataset(root=TEST_DATA_ROOT, freq="h", compute_uv=True)
    ...
    assert isinstance(
        ds.get_observations(split="test", as_numpy=True, return_mask=True), tuple
    )
...
def test_windows_as_xarray():
    ...
    w_size = 24
    h_size = 12
    train_windows_xarray = ds.get_windows(
        window_size=w_size, horizon_size=h_size, split="train", as_xarray=True
    )
    ...
    # assert that reftime + lagtime (broadcasted along the window)
    # corresponds to index_x (lag is negative)
    induced_index_x = (
        train_windows_xarray.x["reftime"].data[:, None]
        + train_windows_xarray.x["lag"].data[None, :]
    )
    ...
    # assert that reftime + leadtime (broadcasted along the horizon)
    # corresponds to index_y (lag is negative)
    induced_index_y = (
        train_windows_xarray.y["reftime"].data[:, None]
        + train_windows_xarray.y["lead"].data[None, :]
    )
    ...
    # check timestamps in train_windows_xarray.x|y are equal to
    # train_windows_xarray.index_x|y via utility timestamps_from_xr
    from peakweather.utils import timestamps_from_xr

    np.testing.assert_array_equal(
        timestamps_from_xr(train_windows_xarray.x, "lag"), train_windows_xarray.index_x
    )
    np.testing.assert_array_equal(
        timestamps_from_xr(train_windows_xarray.y, "lead"), train_windows_xarray.index_y
    )

    # assert that get_observation_windows and get_windows return the same windows
    train_windows_xarray2 = ds.get_observation_windows(
        window_size=w_size, horizon_size=h_size, split="train", as_xarray=True
    )
    for k in ["x", "mask_x", "y", "mask_y"]:
        xr.testing.assert_identical(
            train_windows_xarray.__dict__[k], train_windows_xarray2.__dict__[k]
        )
...
def test_windows_split():
    ...
    train_windows = ds.get_windows(
        window_size=w_size, horizon_size=h_size, split="train"
    )
    ...
    train_windows = ds.get_windows(
        window_size=w_size, horizon_size=h_size, split="train", stations=stations
    )
    ...
    ds = PeakWeatherDataset(
        root=TEST_DATA_ROOT, freq="h", extended_nwp_pars="all", compute_uv=True
    )

    icon_windows = ds.get_windows(
        window_size=w_size,
        horizon_size=h_size,
        split="nwp_test",
        stations=stations,
...
def test_nwp_missing_data():
    dataset = PeakWeatherDataset(
        root=TEST_DATA_ROOT, freq="1h", extended_nwp_pars="all"
    )
...
def test_get_multiple_nwp_in_windows():
    ...
    # assert reftime + lead (shape = [windows,33]) of the NWP is the same as
    # windows.index_y (shape = [windows,33])
    nwp_dates = (windows.nwp.reftime + windows.nwp.lead).values
    y_dates = np.stack(
        [
            pd.to_datetime(windows.index_y[:, j]).tz_localize(None).to_numpy()
            for j in range(windows.index_y.shape[1])
        ],
        axis=1,
    )
...
def test_nwp_alignment():
    ...
    # compute pred error
    def adiff(nwp):
        return np.abs(y - np.median(nwp, 0))
    ...
    def adiff_f(f):
        d = np.abs(windows.y[..., f] - np.nanmedian(windows.nwp, 0))
        return (d * safe_mask).sum()
